...
| Problem | Solution |
|---------|----------|
| **ESP32 won't connect** | Reset ESP32, re-pair Bluetooth, check serial monitor |
| **Game doesn't respond** | Verify BLE connection, check browser console |
| **Too sensitive** | Increase `thresholdOffset` in code |
| **Not sensitive enough** | Decrease `thresholdOffset`, check LDR wiring |
| **Double jumps** | Increase `cooldownTime` value |
| **LEDs don't light** | Check LED polarity and resistor values |
...
1. **Light Detection**: LDR measures ambient light, hand wave creates shadow
2. **Threshold Trigger**: When light change exceeds threshold, obstacle detected
3. **BLE Command**: ESP32 sends SPACE keypress via Bluetooth
4. **Game Response**: Browser receives input, rocket boosts upward
5. **Feedback Loop**: LEDs confirm detection, cooldown prevents spam

### Calibration Algorithm
```
1. Read initial LDR value → baseline
2. Continuous monitoring: current_value vs baseline
3. If |current - baseline| > threshold → trigger
...
- [ ] Mobile app version
- [ ] Additional gesture types (swipe detection)
...
- **PlatformIO** for excellent development platform
...
*Add screenshots of your setup and gameplay here*
